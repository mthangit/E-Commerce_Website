            ...
            <div class="table-responsive text-nowrap">
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu" aria-labelledby="filterDropdown">
                            ...
                            <a class="dropdown-item"
                                href="{{ route('allorder', ['status' => 'processing']) }}">processing</a>
                            <a class="dropdown-item" href="{{ route('allorder', ['status' => 'canceled']) }}">đã hủy</a>
                            <a class="dropdown-item" href="{{ route('allorder', ['status' => 'completed']) }}">đã hoàn
                                thành</a>
                        </div>
                    </div>
                </div>
                <table class="table">
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        @endforeach
                        @if (request()->get('status') == 'date_range')
                                        {{ $orders->appends(['status' => request()->get('status'), 'start_date' => request()->get('start_date'), 'end_date' => request()->get('end_date')])->links() }}
                        @else
                                        {{ $orders->links() }}
                        @endif
                    </tbody>
    ...
    <script>
        $(document).ready(function() {
            $('#filterButton').click(function() {
                ...
                window.location.href = "{{ route('searchorder') }}?status=date_range&start_date=" +
